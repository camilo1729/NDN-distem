...
for formatining later with =btrfs=:

#+BEGIN_SRC sh
 $ oarsub -t deploy -t destructive -l slash_22=1+nodes=5,walltime=5:00:00 "sleep 1d"
#+END_SRC

We need five nodes for deploying the entirerly infrastructure otherwise we will have problems when starting the daemons.
...
In the coodirnator nodes, we need to install the following gems:
